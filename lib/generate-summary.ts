...
12. Do not echo the product title in the opening words. Never repeat three or more consecutive words from the title in the same order at the start of the summary — this makes the summary redundant rather than complementary. For example, if the title is "Personalised Birth Flower Necklace with Birthstone", do not open with "A personalised birth flower necklace with birthstone…" — instead, approach the product from a fresh angle, leading with an observation about how it looks, what it does, or what makes it worth having. The summary should add something the title does not already say.
