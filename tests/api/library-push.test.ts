...
function shopifyPage(nodes: Array<{ id: string; title: string; typePt?: string; stylePt?: string; wct1?: string; pf1?: string }>, hasNextPage = false) {
...
beforeEach(() => {
  ...
  vi.mocked(markPFPushed).mockResolvedValue(undefined);
});
...
describe("POST /api/library/push (WCT)", () => {
  ...
  it("done immediately with zeros when searchFormatted is empty", async () => {
    vi.mocked(getLibraryEdits).mockResolvedValue({
...
describe("POST /api/library/push (PF)", () => {
  it("replaces old phrase with new in matched PF bullets", async () => {
    ...
    vi.mocked(getLibraryEdits).mockResolvedValue({
      wct: {},
      pf: { "pf-1": { id: "pf-1", productType: "ALL", productStyle: "ALL", category: "Occasion", phrase: "New phrase", icon: "home", timeSensitive: null, filterByInterest: false, applicabilityCount: 0, searchPhrase: oldPhrase, isNew: false } },
    });
    vi.mocked(shopifyGraphQL).mockResolvedValue(shopifyPage([
    ...
    const progress = events.filter((e) => e.type === "progress");
    expect(progress[0].status).toBe("updated");
    expect(markPFPushed).toHaveBeenCalled();
  });
});
